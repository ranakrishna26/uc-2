...
import { formatLeaderboardRankPhrase } from './leaderboardAiExplain'

type Props = {
  detail: LeaderboardAiExplainInput
  filterSummary: string
  cell: Cell | null
}

function listModeLabel(mode: LeaderboardAiExplainInput['listMode']): string {
...
function trailBullets(detail: LeaderboardAiExplainInput): { key: string; text: string }[] {
  const { flags } = detail
...
function cellHypothesis(cell: Cell, detail: LeaderboardAiExplainInput): string {
  const { flags } = detail
...
function nextSteps(detail: LeaderboardAiExplainInput, cell: Cell | null): string[] {
  const steps: string[] = []
  const neighborHint =
...
export function LeaderboardExplainCard({ detail, filterSummary, cell }: Props) {
  ...
  return (
    <section className="leaderboard-explain-card" aria-labelledby={`row-insight-title-${detail.cellId}`}>
      <header className="leaderboard-explain-card__header">
        <h3 className="leaderboard-explain-card__title" id={`row-insight-title-${detail.cellId}`}>
          Cell impact row
        </h3>
        <p className="leaderboard-explain-card__subtitle">
          <span className="leaderboard-explain-card__cell-name">{detail.cellName}</span>
          <span className="leaderboard-explain-card__sep" aria-hidden>
            ·
          </span>
          <span className="mono leaderboard-explain-card__cell-id">{detail.cellId}</span>
        </p>
      </header>

      <div className="leaderboard-explain-card__section">
        <h4 className="leaderboard-explain-card__heading">At a glance</h4>
        <dl className="leaderboard-explain-card__stats">
          <div className="leaderboard-explain-card__stat-row">
            <dt>Impacted</dt>
        ...
        {detail.total > 0 ? (
          <div
            className="leaderboard-explain-card__share-bar"
            role="img"
            aria-label={`Impacted share ${sharePct} percent`}
          >
            <div className="leaderboard-explain-card__share-bar-fill" style={{ width: `${sharePct}%` }} />
          </div>
        ) : (
          <p className="leaderboard-explain-card__muted">No subscribers in cohort after filters.</p>
        )}
      </div>

      <div className="leaderboard-explain-card__section leaderboard-explain-card__section--inline">
        <h4 className="leaderboard-explain-card__heading">View</h4>
        <div className="leaderboard-explain-card__chips" aria-label="Lens and list mode">
          <span className="leaderboard-explain-card__chip">Lens: {detail.impactLensLabel}</span>
          <span className="leaderboard-explain-card__chip">{listModeLabel(detail.listMode)}</span>
        </div>
      </div>

      <div className="leaderboard-explain-card__section">
        <h4 className="leaderboard-explain-card__heading">Active filters</h4>
        <ul className="leaderboard-explain-card__filter-list">
          {filters.map((line) => (
            <li key={line}>{line}</li>
          ))}
        </ul>
      ...
      <div className="leaderboard-explain-card__section">
        <h4 className="leaderboard-explain-card__heading">Issue trail</h4>
        <div className="leaderboard-explain-card__trail-row">
          <IssueTrailIcons flags={detail.flags} aggregateScope="cell" className="leaderboard-explain-card__trail-icons" />
        </div>
        {trails.length > 0 ? (
          <ul className="leaderboard-explain-card__trail-bullets">
            {trails.map((t) => (
              <li key={t.key}>{t.text}</li>
            ))}
          </ul>
        ) : (
          <p className="leaderboard-explain-card__muted">
      ...
      {cell ? (
        <div className="leaderboard-explain-card__section">
          <h4 className="leaderboard-explain-card__heading">Cell snapshot</h4>
          <div className="leaderboard-explain-card__fact-grid">
            <span className="leaderboard-explain-card__fact-label">Vendor</span>
            <span>{cell.vendor}</span>
            <span className="leaderboard-explain-card__fact-label">Site / sector</span>
            <span>
              {cell.siteCode} · {cell.sector}
            </span>
            <span className="leaderboard-explain-card__fact-label">Band</span>
            <span>
              {cell.band} ({cell.bandwidthMhz} MHz)
            </span>
            <span className="leaderboard-explain-card__fact-label">PCI / TAC</span>
            <span className="mono">
              {cell.pci} / {cell.tac}
            </span>
            <span className="leaderboard-explain-card__fact-label">Setup / access</span>
            <span className="mono">{cell.setupAccessFailures}</span>
            <span className="leaderboard-explain-card__fact-label">Call drops</span>
            <span className="mono">{cell.callDrops}</span>
            <span className="leaderboard-explain-card__fact-label">DL / UL</span>
            <span className="mono">
              {cell.dlMbps} / {cell.ulMbps} Mbps
            </span>
            <span className="leaderboard-explain-card__fact-label">HO success</span>
            <span className="mono">{cell.hoSuccessPct.toFixed(1)}%</span>
          </div>
          <p className="leaderboard-explain-card__hypothesis">
      ...
      <div className="leaderboard-explain-card__section">
        <h4 className="leaderboard-explain-card__heading">Rank</h4>
        <p className="leaderboard-explain-card__rank-line">{formatLeaderboardRankPhrase(detail)}</p>
      </div>

      <div className="leaderboard-explain-card__section">
        <h4 className="leaderboard-explain-card__heading">Next steps</h4>
        <ol className="leaderboard-explain-card__steps">
      ...
      <div className="leaderboard-explain-card__section leaderboard-explain-card__section--foot">
        <p className="leaderboard-explain-card__footnote">
          Environment label <span className="mono">{detail.environment}</span> is deterministic from the prototype model.
        </p>
        {!detail.fromAnchors ? (
          <p className="leaderboard-explain-card__footnote leaderboard-explain-card__footnote--warn">
